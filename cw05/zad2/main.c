...
//            clock_gettime(CLOCK_REALTIME, &end_time); // get end time
            ...
            if (end_time.tv_nsec < start_time.tv_nsec) {
                elapsed_time.tv_sec--;
                elapsed_time.tv_nsec = 1e9 + end_time.tv_nsec - start_time.tv_nsec;
            } else {
                elapsed_time.tv_nsec = end_time.tv_nsec - start_time.tv_nsec;
            }

            fprintf(report_file, "Process %d took %ld.%09ld seconds\n", getpid(), elapsed_time.tv_sec, elapsed_time.tv_nsec);
